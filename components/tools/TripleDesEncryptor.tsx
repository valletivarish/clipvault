...
export default function TripleDesEncryptor() {
  ...
  const keyValid = key.length === 24;
  const keyColor = keyValid ? '#22C55E' : '#F43F5E';
  ...
  const handleEncrypt = () => {
    ...
    if (!keyValid) {
      setError('Key must be exactly 24 characters for Triple DES');
      return;
    }
    ...
    try {
      ...
      const result = CryptoJS.TripleDES.encrypt(plaintext, CryptoJS.enc.Utf8.parse(key), options).toString();
      setOutput(result);
    } catch (err) {
  ...
  const handleDecrypt = () => {
    ...
    try {
      ...
      const result = CryptoJS.TripleDES.decrypt(ciphertext, CryptoJS.enc.Utf8.parse(key), options).toString(CryptoJS.enc.Utf8);
      if (!result) {
  ...
  return (
    <div className="bg-[#111115] border border-white/[0.06] rounded-xl p-5 space-y-4">
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-[#FAFAFA]">Triple DES Encrypt / Decrypt</h3>
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Enter 24-char key"
                className="w-full bg-[#18181C] border border-white/10 rounded-[7px] px-3 py-2 text-[#FAFAFA] text-sm font-mono outline-none focus:border-[#F97316]/40 transition-colors resize-none"
              />
              <p className="text-[10px] mt-1" style={{ color: keyColor }}>
      ...
      <div className="bg-[#18181C] border border-[#F59E0B]/20 rounded-[7px] p-3">
        <p className="text-[10px] text-[#F59E0B] font-mono">Triple DES is deprecated. Prefer AES-256.</p>
      </div>
    </div>
  );
}
